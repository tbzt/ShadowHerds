...
export const EditionSR5 = {
  ...
  equipPools: {
    ...
    armures: {
      ...
      /* ---- supplements ---- Run & Gun p.223-224 : les armures absentes du Livre de Règles, aux NOMS EXACTS de
         l'annexe pour que `armorReserveFor` retrouve leur capacité propre (ArmuresSR5).
         Généré par REFERENCE/accessoires-armures/genere_pools.py — régénérer. */
      supplements: [
        "Armure anti-émeutes — Casque [+2]",
        "Armure corporelle intégrale — Casque [+3]",
        "Armures de sécurité — Légère [15]",
        "Armures de sécurité — Moyenne [18]",
        "Armures de sécurité — Lourde [20]",
        "Armures de sécurité — Casque [+3]",
        "Armure macabre [13]",
        "Armure militaire renforcée — Casque [+3]",
        "Armure SWAT — Casque [+3]",
        "Combinaison de moto de course — Casque [+2]",
        "Combinaison de pompier — Casque [+2]",
        "Combinaison Urban Explorer — Casque [+2]",
        "Ares Armored Survivalist [8]",
        "Armure de plongée [7]",
        "Bulle de survie [4]",
        "Combinaison Ares Arctic Forces [15]",
        "Combinaison contre le froid blindée Ares [9]",
        "Combinaison de camouflage polaire Ares [6]",
        "Combinaison de plongeur arctique [1]",
        "Combinaison de survie polaire [6]",
        "Combinaison Evo HEL [8]",
        "Combinaison Mitsuhama EE [6]",
        "Combinaison spatiale [12]",
        "Combinaison spatiale de sécurité [15]",
        "Combinaison spatiale blindée Evo Armadillo [16]",
        "Tenue désert [3]",
        "Tenue Ghillie [4]",
      ],
    },
  ...
  /* ---- Attributs de membre aux jets (Livre de Règles p.458) ----
     « Quand un membre en particulier est utilisé pour un test (tel que porter une attaque avec un bras cybernétique), les attributs de ce membre sont utilisés ; dans les autres cas la moyenne des attributs des membres utilisés pour ce test est utilisée. Si une tâche requiert une coordination fine entre plusieurs membres, alors l'attribut du membre le plus faible est utilisé. »
     Le meneur désigne ce qui agit — `pnj.membreActif` : rien (le corps), un
     membre (son index dans l'équipement), « moyenne », « faible » — et
  ...
  cyberlimbsOf(pnj) {
    ...
    (pnj.equip || []).forEach((it, idx) => {
      ...
      const hit = Implants.lookup(this._implantIdx, nom);
      if (!hit || !hit.ref || !/^cybermembres$/i.test(hit.ref.categorie || "")) return;
  ...
  limbOptions(pnj) {
    ...
    opts.push({ value: "moyenne", label: "Moyenne des membres", chosen: cur === "moyenne" });
    opts.push({ value: "faible", label: "Coordination fine (le plus faible)", chosen: cur === "faible" });
    return opts;
  },
