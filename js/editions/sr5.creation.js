...
Object.assign(EditionSR5, {
  creation: {
    ...
    /* ---- Gammes d'implants (Livre de Règles p.451 ; p.94 : « seules les gammes d'occasion, standard et alphaware sont disponibles lors de la création ») ----
    ...
    IMPLANT_GRADES: {
      occasion: { label: "D'occasion", essence: 1.25, cout: 0.75, dispo: -4 },
      standard: { label: "Standard", essence: 1, cout: 1, dispo: 0 },
      alphaware: { label: "Alphaware", essence: 0.8, cout: 1.2, dispo: 2 },
      betaware: { label: "Betaware", essence: 0.7, cout: 1.5, dispo: 4 },
      deltaware: { label: "Deltaware", essence: 0.5, cout: 2.5, dispo: 8 },
    },
    /** Les gammes qu'on peut prendre à la création. */
    IMPLANT_GRADES_AT_CREATION: ["occasion", "standard", "alphaware"],
    ESSENCE_MAX: 6,
    ...
    /* ---- Attributs propres des cybermembres (Livre de Règles p.458-459) ----
       Un membre (hors crâne et torse) a sa Force et son Agilité : base
       3, personnalisable au gabarit du client (« chaque augmentation augmente la Disponibilité » : +1 et +5 000 ¥ par point, jusqu'au maximum naturel), plus les améliorations LOGÉES dans ce membre (Force, Agilité, Armure : une de chaque type par membre).
    ...
    CYBERLIMB: { base: 3, personnalisation: { cout: 5000, dispo: 1 } },

    isCyberlimb(gear) {
      const d = this.isImplant(gear) ? this.implantDefaults(gear.name, gear.rating) : null;
      if (!d || !d.ref || !/^cybermembres$/i.test(d.ref.categorie || "")) return false;
    ...
    isImplant(gear) {
      return !!(gear && gear.kind && EditionSR5.AUGS_KEYS.includes(gear.kind));
    },
    ...
    _tableImplantGroup(objets) {
      const AUGS = EditionSR5.AUGS_KEYS;
      const deja = new Set();
