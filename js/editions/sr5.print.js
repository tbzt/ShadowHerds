...
/* ============================================================
   FEUILLE IMPRIMABLE SR5 — fidèle au bloc d'archétype du Livre de
   Règles (« dossier classifié ») : barre-titre, MÉTATYPE, table
   d'attributs à en-têtes ambre, lignes « Libellé : … », moniteurs
   entiers. Compagnon de l'édition SR5 (savoir spécifique confiné
...
const PrintSR5 = {
  ...
  _attrTable(pnj) {
    const a = pnj.attrs || {};
    const cols = [
      ["CON", a.CON], ["AGI", a.AGI], ["RÉA", a.REA], ["FOR", a.FOR],
      ["VOL", a.VOL], ["LOG", a.LOG], ["INT", a.INT], ["CHA", a.CHA],
      ["ESS", a.ESS != null ? a.ESS : "—"],
    ];
  ...
  build(pnj) {
    ...
    const augsKeys = EditionSR5.AUGS_KEYS;
    ...
    const metaStr =
      pnj.meta + (pnj.metavariant ? ` (${this._esc(pnj.metavariant)})` : "");

    const main = [
      this._attrTable(pnj),
      this._line("Moniteur de condition (P/E)", `${this._esc(pnj.physMon)} / ${this._esc(pnj.stunMon)}`),
      this._monitors(pnj),
      this._line("Armure", pnj.armure != null ? this._esc(pnj.armure) : ""),
      this._line("Limites", `Physique ${this._esc(pnj.limPhys)}, Mentale ${this._esc(pnj.limMent)}, Sociale ${this._esc(pnj.limSoc)}`),
      this._line("Initiative", `${this._esc(pnj.init)} + ${this._esc(pnj.initDice)}D6`),
