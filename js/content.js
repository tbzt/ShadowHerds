...
const Content = {
    ...
    sr6: {
      ...
      heavy: [
        {
          ...
          proRatingMin: 1,
          forTags: ["combat", "ombre", "crime", "militaire", "police"],
        },
        {
          name: "Colt Manhunter",
          stats: "VD 3P, SA, 16(c)",
          proRatingMin: 1,
          forTags: ["corpo", "police", "militaire"],
        },
        {
          name: "Ruger Super Warhawk",
          stats: "VD 3P, CA, 6(cy)",
          proRatingMin: 2,
          forTags: ["combat", "crime", "gang"],
        },
      ],
      rifle: [
        {
      ...
      melee: [
        ...
        },
        {
          ...
          stats: "VD (FOR+3)P",
          proRatingMin: 2,
          forTags: ["combat", "crime", "ombre", "militaire"],
        },
        {
          name: "Matraque télescopique",
          stats: "VD (FOR+2)E",
          proRatingMin: 0,
          forTags: ["police", "corpo", "rue"],
        },
      ],
    },
  ...
  /** Choisit une arme cohérente forTags l'édition, l'archétype et le proRating. */
  pickWeapon(ed, tags, proRating) {
    ed = this._ed(ed);
    const weapons = this.weapons[ed] || {};
    let cats;
    if (tags.has("militaire") || tags.has("combat"))
      cats = ["rifle", "heavy", "melee"];
    else if (tags.has("gang")) cats = ["heavy", "rifle", "melee"];
    else if (tags.has("ombre") || tags.has("crime"))
      cats = ["heavy", "light", "melee"];
    else if (tags.has("police") || tags.has("corpo"))
      cats = ["heavy", "rifle", "melee"];
    else if (tags.has("magique")) cats = ["light", "melee"];
    else cats = ["light", "melee"];

    let pool = [];
    for (const c of cats) {
      pool = pool.concat(this._eligible(weapons[c] || [], proRating, tags));
    }
    if (!pool.length) pool = this._eligible(weapons.light || [], proRating, tags);
    if (!pool.length) return null;
    const weapon = Utils.rand(pool);
    // Renvoie une chaîne « Nom [stats] » affichée telle quelle dans
    // l'Équipement (pas de modale, VD directement lisible).
    return weapon.stats ? `${weapon.name} [${weapon.stats}]` : weapon.name;
  },
