...
export const CardRenderer = {
  ...
  /** CO-d : `data-id="${c.id}"` explicite sur chaque champ éditable — sur la
      ...
      (cf. `CardDelegation.bindDelegation`), jamais l'ancêtre `.pnj-card`. */
  _contactStatsSR(c) {
    ...
    return `<div class="cluster stats-row" style="margin-top:6px;flex-wrap:wrap;gap:6px;">
      ...
      ${c.lieu ? `<span style="font-size:var(--fs-xs);color:var(--text-dim);align-self:center;"><span class="card-section-label">Lieu</span> ${this._esc(c.lieu)}</span>` : ""}
    </div>`;
  },
  ...
  /** Les TROIS pièces de la ligne d'états, séparées — pour une surface qui
      ...
      Même balisage, même délégation (`CardDelegation` la pose sur `document`,
