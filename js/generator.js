...
const Gen = {
  ...
  _formHTML(ed, prefix, opts = {}) {
    ...
    // Professionnalisme (SR5/SR6) ou Rang (Anarchy)
    if (fo.rang) {
  ...
  clearSingle() {
    const zone = document.getElementById('gen-zone-single');
    const ids = [...zone.querySelectorAll('.pnj-card')].map(c => c.dataset.id);
  ...
  showTab(tab, btn) {
    document.querySelectorAll('#panel-generator .tab-btn')
      .forEach(b => b.classList.remove('active'));
    document.querySelectorAll('#panel-generator .tab-panel')
