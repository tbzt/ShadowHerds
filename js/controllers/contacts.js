...
/* ============================================================
   CONTACTS — générateur de contacts (panneau + données).

   Domaine contact, brique « générateur » : formulaire dirigé, données
   (catalogues Réseaux/métiers) et fabrication d'un objet contact
   (build/buildManual). Miroir du générateur PNJ (Gen). La persistance
   est déléguée à la bibliothèque ContactsBook ; ce fichier ne persiste
   rien lui-même. (L'ancien module ContactGen a été fusionné ici —
   convergence des domaines, cf. AUDIT_ARCHITECTURE CH-A1.)
...
export const Contacts = {
  ...
  _syncCost() {
    ...
    const cost = this.atoutCost(rr, scope);
    ...
    if (descEl && netId) {
      const n = this.NETWORKS.find((x) => x.id === netId);
